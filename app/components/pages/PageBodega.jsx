...
export default function PageBodega({ session, supabase, onLogout }) {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      const q = supabase.from("causas").select("*").in("estado",["acta_recibida","aceptada"]).order("created_at",{ascending:true});
      if (session?.casaId) q.eq("casa_id", session.casaId);
      const { data } = await q;
      setCausas(data||[]);
      setLoading(false);
    };
  ...
  const causa = selected ? causas.find(c=>c.id===selected) : null;
  ...
  const confirmarRecepcion = async () => {
    ...
    // Email a Victor (buscar admin/martillero de la casa)
    try {
      let emailVictor = null;
      if (session?.casaId) {
        const { data:admins } = await supabase.from("usuarios")
          .select("email")
          .eq("casa_id", session.casaId)
          .in("roles", [["admin"],["martillero"]])
          .limit(1);
        if (!admins?.length) {
          const { data:admins2 } = await supabase.from("usuarios")
            .select("email,roles")
            .eq("casa_id", session.casaId)
            .limit(5);
          const a = (admins2||[]).find(u=>Array.isArray(u.roles)&&(u.roles.includes("admin")||u.roles.includes("martillero")));
          if (a) emailVictor = a.email;
        } else emailVictor = admins[0].email;
      }
      if (emailVictor) {
    ...
    setCompletados(prev=>[...prev,{ ...updated, nombreDeudor, firma, fechaHora }]);
    setSaving(false);
    setStep("confirmado");
  };
      ...
      <div style={{ background:"linear-gradient(135deg,#0f4c5c,#0891b2)", padding:"1.2rem 1.5rem", color:"#fff" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div>
            ...
            <div style={{ fontSize:"1.2rem", fontWeight:800, marginTop:".1rem" }}>Causas pendientes</div>
          </div>
      ...
      <div style={{ flex:1, padding:"1rem", display:"flex", flexDirection:"column", gap:".75rem" }}>
        ...
        {causas.map(c=>(
          <div key={c.id} onClick={()=>{ setSelected(c.id); setNombreDeudor(""); setRutDeudor(""); setFirma(null); setStep("recepcion"); }}
            style={{ background:"#fff", borderRadius:14, padding:"1.1rem 1.3rem", boxShadow:"0 2px 8px rgba(0,0,0,.08)", cursor:"pointer", border:"1.5px solid #e5e7eb", active:{borderColor:"#0891b2"} }}>
