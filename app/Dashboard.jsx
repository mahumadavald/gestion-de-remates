    ...
    const cargar = async () => {
      ...
      try {
        ...
        if (mounted) {
          ...
          if (causasRes?.data)  setDbCausas(causasRes.data);
...
<\/script>
  ...
  const printCatalogoPDF = async () => {
    ...
    try {
      ...
      doc.save(`catalogo-${remateNombre.replace(/[^a-zA-Z0-9]/g, "-").toLowerCase()}.pdf`);
  ...
  const _ctx = {
    session, notify, notif, supabase, authFetch,
    page, setPage,
    modal, setModal,
    ...
    actualizarLicencia, renovarLicencia, cambiarPlan, guardarNota, subirLogoCasa,
  };

  return (
    <DashboardContext.Provider value={_ctx}>
    <div className="app">
      <style>{CSS}</style>
      ...
      <aside className={`sidebar${mobileMenu?" open":""}${sidebarCollapsed?" collapsed":""}`} style={page==="sala"?{display:"none"}:undefined}>
        ...
        <nav className="sb-nav">
          ...
          <div className={`sb-item${page==="causas"?" on":""}`} onClick={()=>{setPage("causas");setMobileMenu(false);}}>
            ...
            <span className="sb-label">Causas</span>
            {dbCausas.filter(c=>["notificada","aceptada","acta_recibida","bienes_recepcionados","fecha_solicitada","fecha_aprobada"].includes(c.estado)).length > 0 && (
              <span className="sb-badge">{dbCausas.filter(c=>["notificada","aceptada","acta_recibida","bienes_recepcionados","fecha_solicitada","fecha_aprobada"].includes(c.estado)).length}</span>
            )}
          </div>
      ...
      <div className="main-wrap">
        ...
        {/* ══ POSTORES ══ */}
        {(page==="postores"||page==="clientes") && <PagePostores/>}
        ...
        {/* ══ VENDEDORES / CONSIGNATARIOS ══ */}
        {page==="vendedores" && <PageVendedores/>}
        ...
        {/* ══ USUARIOS ══ */}
        {page==="usuarios" && session?.role==="admin" && <PageUsuarios session={session} supabase={supabase} dbBodegas={dbBodegas} dbLicencias={dbLicencias} usuarios={usuarios} setUsuarios={setUsuarios} notify={notify}/>}

        {/* ══ LICENCIAS ══ */}
        {page==="licencias" && session?.role==="admin" && <PageLicencias dbLicencias={dbLicencias} actualizarLicencia={actualizarLicencia} renovarLicencia={renovarLicencia} cambiarPlan={cambiarPlan} guardarNota={guardarNota}/>}
        ...
        {/* ══ LIQUIDACIONES ══ */}
        {page==="liquidac" && <PageLiquidaciones/>}
        ...
        {/* ══ DEVOLUCIONES ══ */}
        {page==="devoluciones" && <PageDevoluciones/>}
